...
export class ChatGateway implements OnGatewayConnection, OnGatewayDisconnect {
    ...
    async handleGetClashes(@MessageBody() data: { channelId: number },@ConnectedSocket() client: Socket) {
        const user = (client as any).user;
        ...
        const clashes = await this.chatService.getClashsByChannel(data.channelId);
    ...
    async handleCreateClash(@MessageBody() data: { createClashDto: CreateClashDto },@ConnectedSocket() client: Socket) {
        ...
        this.server.to(`channel_${data.createClashDto.channelId}`).emit('newClash', clash);
        client.emit('clashSent', clash);
    }

    @SubscribeMessage('submiteVote')
    async handleVoteClash(@MessageBody() data: { voteDto: VoteDto },@ConnectedSocket() client: Socket) {
        const user = (client as any).user;
        ...
        this.server.to(`channel_${vote.clash.channel.id}`).emit('newVote', vote);
        client.emit('voteSent', vote);
    }
}